...
class TestPendingInviteAuthorization:
    ...
    async def test_allowed_inviter_is_joined(self):
        ...
        sync_data = _invite_sync_data(
            invite_state={"events": [_member_invite_event()]}
        )
        adapter._schedule_pending_invite_joins(sync_data)
        await _drain_invite_tasks(adapter)

        adapter._join_room_by_id.assert_awaited_once_with(
            "!pending_room:example.org"
        )
    ...
    @pytest.mark.asyncio
    async def test_unauthorized_or_unknown_inviter_is_not_joined(
        self, invite_state
    ):
